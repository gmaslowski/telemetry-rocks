...
object ByteStringReaderSimpleTypes {
  implicit class ByteStringTelemetryPacketSimple(val packet: ByteString) {

    def getFloat: Int => Float = idx => packet.slice(idx, idx + 4).reverse.asByteBuffer.getFloat
    def getByte: Int => Byte = idx => packet.asByteBuffer.get(idx)
